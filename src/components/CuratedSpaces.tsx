...
function ProductPreviewModal({ product, onClose }: ProductPreviewModalProps) {
  ...
  // Keyboard navigation: Escape closes modal, Left/Right arrow keys navigate images
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        onClose();
      } else if (e.key === "ArrowLeft" && emblaApi && hasMultipleImages) {
        emblaApi.scrollPrev();
      } else if (e.key === "ArrowRight" && emblaApi && hasMultipleImages) {
        emblaApi.scrollNext();
      }
    };
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-3 right-3 z-30 flex h-9 w-9 items-center justify-center rounded-full bg-black/65 text-white sm:bg-white/90 sm:text-neutral-600 shadow-md backdrop-blur-xs transition-colors hover:bg-black/85 sm:hover:bg-white hover:text-white sm:hover:text-black focus-visible:ring-2 focus-visible:ring-brand-slate-deep cursor-pointer border border-white/20 sm:border-0"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 overflow-y-auto lg:overflow-hidden h-full sm:h-auto max-h-[100dvh] sm:max-h-[90vh] lg:max-h-[88vh] lg:h-[620px] scrollbar-thin-brass flex-1">
          ...
          <div className="lg:col-span-7 flex flex-col justify-between bg-neutral-900 p-3 sm:p-5 lg:p-4 xl:p-5 relative select-none shrink-0">
            {/* Embla Viewport */}
            <div className="relative flex-1 flex items-center justify-center overflow-hidden rounded-none sm:rounded-xl bg-neutral-950/80 min-h-[250px] sm:min-h-[320px] lg:min-h-[360px]">
              <div className="overflow-hidden w-full h-full" ref={emblaRef}>
                <div className="flex h-full touch-pan-y">
                  {images.map((src, idx) => (
                    <div
                      key={idx}
                      className="relative min-w-full flex-1 flex items-center justify-center aspect-[4/3] sm:aspect-[16/11]"
                    >
                      <Image
                        src={src}
              ...
              {hasMultipleImages && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/60 text-white backdrop-blur-xs transition-all hover:bg-black/90 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/60 text-white backdrop-blur-xs transition-all hover:bg-black/90 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              ...
              {/* Image Counter Badge */}
              {hasMultipleImages && (
                <div className="absolute bottom-3 right-3 rounded-full bg-black/70 px-2.5 py-1 text-[11px] font-semibold text-white backdrop-blur-xs">
            ...
            {/* Thumbnail Strip */}
            {hasMultipleImages && (
              <div className="mt-2.5 sm:mt-3 flex gap-2 overflow-x-auto pb-1 pt-0.5 scrollbar-thin-brass justify-start sm:justify-center">
                {images.map((src, idx) => (
                  <button
                    key={idx}
                    type="button"
                    onClick={() => scrollTo(idx)}
                    className={`relative h-11 w-15 sm:h-13 sm:w-17 shrink-0 overflow-hidden rounded-xs border-2 transition-all cursor-pointer bg-white ${
                      selectedIndex === idx
                        ? "border-accent-brass shadow-sm scale-105"
          ...
          <div className="lg:col-span-5 p-4 sm:p-5 lg:p-5 xl:p-6 flex flex-col justify-between bg-white overflow-y-auto scrollbar-thin-brass space-y-3 pb-8 sm:pb-5 safe-bottom">
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2 border-b border-neutral-100 pb-2">
                <span className="text-[11px] sm:text-xs font-bold text-accent-brass-dark uppercase tracking-wider">
                  {lang === "en" ? `${categoryLabel} Collection` : `${categoryLabel} কালেকশন`}
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 sm:px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-text-primary-dark">
                    {t("পছন্দের পলিশ ফিনিশ:", "Wood Finish Color:")}
                  </span>
                  <span className="text-[11px] font-semibold text-accent-brass-dark">
                    {finishName}
                  </span>
                </div>
              ...
              <p className="text-[11px] text-text-secondary-dark flex items-center gap-1.5">
                <span className="text-emerald-600 font-bold">✓</span>
                <span>{t("আগ্রাবাদ শোরুমে সরাসরি এসে কাঠের ফিনিশ দেখে অর্ডার করতে পারেন", "Visit our Agrabad showroom to inspect finishes in person")}</span>
              </p>
            </div>
